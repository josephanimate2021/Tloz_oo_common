...
def create_items(world) -> None:
    """
    Creates items for a game.

    Parameters:
        world: The world that is being used for the creation.
    """
    item_pool_dict = build_item_pool_dict(world)
    items = []
    for item_name, quantity in item_pool_dict.items():
        for _ in range(quantity):
            if type(item_name) == tuple:
                for i in range(len(item_name)):
                    items.append(create_item(world, item_name[i]))
            else:
...
def build_item_pool_dict(world) -> dict[str, int]:
    """
    Builds an item pool for a game.

    Parameters:
        world: The world that is being used for the building.

    Returns:
        Dict: A fully built item pool.
    """
    excluded_mapass = set()
    ...
    for loc_name, loc_data in LOCATIONS_DATA.items():
        ...
        if item_name.startswith("Rupees ("):
            if world.options.shop_prices == OraclesShopPrices.option_free:
                ...
                rupee_item_count += 1
            continue
        if world.seasons and item_name.startswith("Ore Chunks ("):
    ...
    if world.options.cross_items:
        if world.seasons:
            ...
            item_pool_dict["Progressive Feather"] = 2
            item_pool_dict["Rod of Seasons"] = 1
            item_pool_dict["Fool's Ore"] = 1
            item_pool_dict["Progressive Slingshot"] = 2
            extra_items += 6
    ...
    if world.seasons and ore_item_count > 0:
...
def pre_fill_seeds(world) -> None:
    """
    Fills the seed trees with actual seeds.

    Parameters:
        world: The world used for the filling of the seed trees.
        
    The prefill algorithm for seeds has a few constraints:
      - it needs to place the "default seed" into Horon Village (or Lynna City if the game is Ages) seed tree
      - it needs to place a random seed on the "duplicate tree" (can be Horon (or Lynna if the game is Ages)'s tree)
      - it needs to place one of each seed on the 5 remaining trees
    This has a few implications:
      - if Horon (or Lynna if the game is Ages) is the duplicate tree, this is the simplest case: we just place a starting seed in Horon's tree
        and scatter the 5 seed types on the 5 other trees
      - if Horon (or Lynna if the game is Ages) is NOT the duplicate tree, we need to remove Horon (or Lynna if the game is Ages)'s seed from the pool of 5 seeds to scatter
        and put a random seed inside the duplicate tree. Then, we place the 4 remaining seeds on the 4 remaining
        trees
    """
    duplicate_tree_name = world.trees_table[world.options.duplicate_seed_tree.value]
    ...
    trees_to_process = [name for name in world.trees_table.values() if name not in manually_placed_trees]
